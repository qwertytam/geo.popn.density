...
fpath_popn <- here(extdata_dir, fname_popn)

url_base <- "https://www.abs.gov.au/"
url_popn <- paste(url_base, "statistics/people/population/regional-population/2020-21/", sep="")
url_popn <- paste(url_popn, fname_popn, sep="")
...
fpath_sa2_sua <- here(extdata_dir, fname_sa2_sua)

url_sa2_sua <- paste(url_base, "ausstats/subscriber.nsf/", sep="")
url_sa2_sua <- paste(url_sa2_sua, "log?openagent&1270055004_sa2_sua_2016_aust_csv.zip", sep="")
url_sa2_sua <- paste(url_sa2_sua, "&1270.0.55.004&Data%20Cubes", sep="")
url_sa2_sua <- paste(url_sa2_sua, "&D6E51168BD6DC248CA2581B1000E0A48&0", sep="")
url_sa2_sua <- paste(url_sa2_sua, "&July%202016&09.10.2017&Latest", sep="")

res <- tryCatch(download.file(url_sa2_sua, fpath_sa2_sua), error = function(e) 1)

# Unzip the download and remove the zip file
flist <- unzip(fpath_sa2_sua, list = TRUE)
unzip(fpath_sa2_sua, exdir = here(extdata_dir))
...
fpath_sa2_sua <- here(extdata_dir, flist[[1]])
...
fname_out <- "au.csv"
fpath_out <- here(extdata_dir, fname_out)
tbl_df %>%
  write_csv(fpath_out)
